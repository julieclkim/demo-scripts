(function () {
  ...
  let studentRecord = {};
  let dataLookupAttempted = false;
  let dataLookupStatus = "not-started";
  ...
  const defaults = {
    ...
    student_notes: "Student record loaded from Genesys Cloud participant attributes or demo lookup data.",
  ...
  function getKeyFromQuery() {
    return clean(rawQueryValue("key"));
  }

  async function loadStudentRecord() {
    const key = getKeyFromQuery();
    if (!key) return;
    dataLookupAttempted = true;
    try {
      const response = await fetch(DATA_URL, { cache: "no-store" });
      if (!response.ok) throw new Error("Unable to load student lookup data");
      const data = await response.json();
      studentRecord = data[key] || {};
      dataLookupStatus = Object.keys(studentRecord).length ? "found" : "not-found";
    } catch (error) {
      dataLookupStatus = "error";
      studentRecord = {};
      console.warn("Student lookup failed", error);
    }
  }

  function unresolvedQueryFields() {
    const fields = [];
    for (const [key, value] of params.entries()) {
      if (unresolvedToken(value)) fields.push(key);
    }
    return fields;
  }

  function renderDiagnostics() {
    const unresolved = unresolvedQueryFields();
    const showLookupMiss = dataLookupAttempted && dataLookupStatus !== "found";
    if (!unresolved.length && !showLookupMiss) return;
    ...
    if (unresolved.length) {
      messages.push(`Genesys has not resolved these URL values: ${unresolved.join(", ")}. Insert the script variable token from the Genesys variable picker, or map the value as a Screen Pop input.`);
    }
    if (showLookupMiss && getKeyFromQuery()) {
      messages.push(`No matching demo record was found for key ${getKeyFromQuery()}. Confirm that the collected student ID matches a key in data/students.json.`);
    }
    if (dataLookupStatus === "error") {
      messages.push("The page could not load data/students.json. Confirm the data folder is published to GitHub Pages.");
    }
